...
class ExChatGPT:
    ...
    def load_recent_history(self, user_id: str, convo_id: str = "default"):
        ...
        create_tables(conn)
        recent_history = get_user_recent_history(conn=conn,
                                                 user_id=user_id,
                                                 convo_id=convo_id,
                                                 limit=100)
        if conn:
    ...
    def add_to_conversation(self,
        ...
        if (user_id not in self.conversation
                or convo_id not in self.conversation[user_id]):
            self.reset(user_id=user_id, convo_id=convo_id)
        self.conversation[user_id][convo_id].append({
            "role": role,
            "content": message
        })

    def add_to_db_temp(self,
                       user_id: str,
                       message: str,
                       role: str,
                       convo_id: str = "default"):
        ...
        self.new_add_conversation[user_id].append({
            "convo_id": convo_id,
            "role": role,
            "content": message,
            "time": created_time
    ...
    def __truncate_conversation(self, user_id: str, convo_id: str = "default"):
        ...
        self.conversation[user_id][convo_id] = self.conversation[user_id][
            convo_id][:-1]
        full_conversation = "\n".join(
            [x["content"] for x in self.conversation[user_id][convo_id]], )
        ...
        while True:
            if (len(ENCODER.encode(query + full_conversation)) >
                    self.max_tokens):
                query = query[:-self.decrease_step]
            else:
                break
    ...
    def ask_stream(
        ...
        **kwargs,
        ...
        if user_id not in self.conversation or convo_id not in self.conversation[
                user_id]:
            self.reset(user_id=user_id, convo_id=convo_id)
    ...
    def delete_last2_conversation(self,
                                  user_id: str,
                                  convo_id: str = "default"):
